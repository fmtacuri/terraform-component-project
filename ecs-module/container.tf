...
@app.route(\"/\")
def hello():
  return \"<!DOCTYPE html><html><head><title>Terraform Tacuri Example</title><h1>AWS EC2 Tag Instances:</h1></head><body>${ join(var.separator, null_resource.instances.*.triggers.name) }</body></html>\"
